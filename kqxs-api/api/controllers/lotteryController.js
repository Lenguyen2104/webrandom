...
exports.updateLotteryData = (req, res) => {
  ...
  // Kiểm tra tính hợp lệ của từng phần tử trong dữ liệu
  ...
  for (const item of data) {
    if (
      ...
      item.numbers.some((num) => typeof num !== "string")
    ) {
      return res.status(400).json({ error: "Dữ liệu không hợp lệ" });
    }
  }
  ...
  connection.query(
    "SELECT data FROM lottery_stages LIMIT 1",
    (err, results) => {
      if (err) {
        return res.status(500).json({ error: "Lỗi máy chủ" });
      }
      ...
      if (results.length === 0) {
        return res.status(404).json({ error: "Không tìm thấy dữ liệu xổ số" });
      }
      ...
      // Cập nhật dữ liệu trong cơ sở dữ liệu
      const updateQuery =
        "UPDATE lottery_stages SET data = ?, updated_at = NOW() LIMIT 1";
      ...
      connection.query(updateQuery, values, (err, results) => {
        ...
        // Truy vấn lại dữ liệu mới cập nhật
        connection.query(
          "SELECT data FROM lottery_stages LIMIT 1",
          (err, results) => {
            if (err) {
              return res
                .status(500)
                .json({ error: "Lỗi máy chủ khi truy vấn dữ liệu mới" });
            }

            if (results.length === 0) {
              return res
                .status(404)
                .json({ error: "Không tìm thấy dữ liệu xổ số" });
            }

            let updatedData = results[0].data;
            ...
            if (typeof updatedData === "string") {
              try {
                ...
                return res
                  .status(500)
                  .json({ error: "Dữ liệu mới không hợp lệ" });
              }
            }

            broadcastLotteryDataToUsers(updatedData);

            // Phản hồi thành công với dữ liệu mới
